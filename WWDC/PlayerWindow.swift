...
open class PlayerWindow: NSWindow {
    ...
    open override var effectiveAppearance: NSAppearance {
        return WWDCAppearance.appearance()
    }
    ...
    fileprivate func appearanceForWidgets() -> NSAppearance? {
        if allowsPiPMode {
            return NSAppearance(appearanceNamed: "PiPZoom", bundle: Bundle(for: PlayerWindow.self))
        } else {
            return WWDCAppearance.appearance()
        }
    }
    ...
    open override var contentView: NSView? {
        set {
            let darkContentView = PlayerWindowContentView(frame: newValue?.frame ?? NSZeroRect)
...
private class PlayerWindowContentView: NSView {
...
private class PlayerWindowOverlayView: NSView {
    ...
    fileprivate var PlayerWindow: PlayerWindow? {
        return window as? PlayerWindow
    }
    ...
    fileprivate override func mouseUp(with event: NSEvent) {
        ...
        if event.clickCount == 2 {
            self.PlayerWindow?.reallyDoToggleFullScreenImNotEvenKiddingItsRealThisTimeISwear(self)
        }
    }
